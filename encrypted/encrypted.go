...
func NewCrypter(scheme aead.Scheme, key []byte) (*Crypter, error) {
	if len(key) != scheme.KeySize() {
		return nil, aead.ErrKeySize
	}
	return &Crypter{Scheme: scheme, Key: key}, nil
}
...
func (c *Crypter) Encrypt(nonce, ad []byte) (aead.Cipher, error) {
	return c.Scheme.Encrypt(c.Key, nonce, ad), nil
}
...
func (c *Crypter) Decrypt(nonce, ad []byte) (aead.Cipher, error) {
	return c.Scheme.Decrypt(c.Key, nonce, ad), nil
}
