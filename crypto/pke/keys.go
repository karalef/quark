...
type rawPublicKey interface {
	...
	Encrypt(plaintext, seed []byte) (ciphertext []byte, err error)
}
...
func (k publicKey) Encrypt(pt, seed []byte) (ciphertext []byte, err error) {
	return k.PublicKey.Encrypt(pt, seed)
}
